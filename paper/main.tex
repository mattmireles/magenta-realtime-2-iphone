...
\item A corrected liveness contract that distinguishes continuous playback from interactive steering. The current runtime sustains output, but its ordinary 6.48-9.02 second audible control latency is not yet a strong live-product result.
\end{itemize}
...
Every public result is generated from a machine-readable receipt. Raw device WAVs, events, tokens, and traces remain private because the product runtime is private, but their SHA-256 hashes, normalized summaries, commands, model identities, and source hashes are public. Precision experiments stop in the order finite, deterministic parity, device latency, then audio; a wrong function never earns a favorable speed number.
