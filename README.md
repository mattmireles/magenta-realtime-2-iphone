...
Superseded generation (retained as negative-result artifacts, still mirrored on
HF): `convert_temporal_body.py` (stateful fp16), `convert_depth_body.py` (fp32
logits + host sampling), and `convert_spectrostream_decoder.py` with its FP32
defaults. Both superseded exporters are headed as superseded with pointers to
the corrected ones and the paper findings.
...
provenance sidecar next to its `.mlpackage`. The sha256 checksums in
[MODELS.md](MODELS.md) are for the **superseded** binaries currently mirrored on
Hugging Face; the corrected binaries are reproduced by the commands above and
are pending upload to HF.
